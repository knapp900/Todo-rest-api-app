...
import by.ak.todo_restapi_app.exceptions.TaskListNotFoundException;
import by.ak.todo_restapi_app.exceptions.TasksListServiceException;
import by.ak.todo_restapi_app.exceptions.UserNotFoundException;
...
public class TasksListServiceImpl implements TasksListService {
    ...
    public TasksList updateTasksList(Long id, String description, String username) {
        try {
            TasksList tasksListForUpdating = this.tasksListRepository.findAllByUser_UsernameAndId(username, id).orElseThrow(() -> {
    ...
    public TasksList getTasksListByUsernameAndId(Long id, String username) {
        try {
            return this.tasksListRepository.findAllByUser_UsernameAndId(username, id).orElseThrow(() -> {
                log.error("Error task list with id: {} not found", id);
                return new TaskListNotFoundException(String.format("Task list with id: %d nut found."));
            });

        } catch (Exception e) {
            log.error("Error fetching task list by id:{}", id, e);
            throw new TasksListServiceException(String.format("Fetching task list by id: %s failed", id));
        }
    }
    ...
    public Page<TasksList> getAllTasksLists(Pageable pageable, String username) {
        try {
            return this.tasksListRepository.findAllByUser_Username(username, pageable);

        } catch (Exception e) {
            log.error("Error fetching all task lists for user: {}.", username);
            throw new TasksListServiceException(
                    String.format("Fetching all task lists for user: %s failed."));
        }
    }
    ...
    public Page<TasksList> getAllUncompletedTasksLists(Pageable pageable, String username) {
        try {
            return this.tasksListRepository.findAllByUser_UsernameAndIsCompleteFalse(username, pageable);

        } catch (Exception e) {
            log.error("Error fetching all uncompleted task lists for user: {}.", username);
            throw new TasksListServiceException(
                    String.format("Fetching all uncompleted task lists for user: %s failed."));
        }
    }
    ...
    public Page<TasksList> getAllCompletedTasksLists(Pageable pageable, String username) {
        try {
            return this.tasksListRepository.findAllByUser_UsernameAndIsCompleteTrue(username, pageable);

        } catch (Exception e) {
            log.error("Error fetching all completed task lists for user: {}.", username);
            throw new TasksListServiceException(
                    String.format("Fetching all completed task lists for user: %s failed."));
        }
    }

    protected TasksList getById (Long id) {
        return this.tasksListRepository.findById(id).orElseThrow(()-> {
            log.error("Error task list with id: {} not found", id);
            return new TaskListNotFoundException(String.format("Task list with id: %d nut found."));
        });
    }
    protected void save(TasksList tasksList){
        this.save(tasksList);
    }
}
